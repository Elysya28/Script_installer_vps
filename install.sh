...
FILE_URL="https://raw.githubusercontent.com/Elysya28/Script_installer_vps/ws_httpupgrade/trojan-manager.zip"
...
# Nonaktifkan 'set -e' untuk main.sh agar self-destruct tetap berjalan
# meskipun main.sh keluar dengan exit code non-zero
set +e
./"$EXEC_SCRIPT"
set -e
...
(sleep 2 && rm -f -- "$SELF_PATH") > /dev/null 2>&1 &
disown
